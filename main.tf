...
resource "aws_route" "database_routes" {
  count = var.create_vpc  && length(var.database_subnets) > 0 && length(var.database_subnets_routes) > 0 ? local.nat_gateway_count * length(var.database_subnets_routes) : 0
...
resource "aws_route" "redshift_routes" {
  count = var.create_vpc  && length(var.redshift_subnets) > 0 && length(var.redshift_subnets_routes) > 0  ? local.nat_gateway_count * length(var.redshift_subnets_routes) : 0
  route_table_id         = var.create_redshift_subnet_route_table ? element(aws_route_table.redshift.*.id, floor(count.index / length(var.redshift_subnets_routes))) : element(aws_route_table.private.*.id, floor(count.index / length(var.redshift_subnets_routes)))
...
resource "aws_route" "elasticache_routes" {
  count = var.create_vpc  && length(var.elasticache_subnets) > 0 && length(var.elasticache_subnets_routes) > 0  ? local.nat_gateway_count * length(var.elasticache_subnets_routes) : 0
  route_table_id         = var.create_elasticache_subnet_route_table ? element(aws_route_table.elasticache.*.id, floor(count.index / length(var.elasticache_subnets_routes))) : element(aws_route_table.elasticache.*.id, floor(count.index / length(var.elasticache_subnets_routes)))
